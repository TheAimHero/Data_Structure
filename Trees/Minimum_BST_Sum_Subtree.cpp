...
class Solution {
public:
  int f(Node *root, int &minlen, int &sum, int target, bool &flag, int &maxi,
        int &mini) {
    if (root == NULL)
      return 0;
    ...
    int leftlen =
        f(root->left, minlen, leftsum, target, leftflag, leftmaxi, leftmini);
    int rightlen = f(root->right, minlen, rightsum, target, rightflag,
                     rightmaxi, rightmini);
    ...
    flag = false;
    if (root->data > leftmaxi && root->data < rightmini && leftflag &&
        rightflag) {
